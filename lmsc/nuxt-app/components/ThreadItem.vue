  ...
  <v-list-item class="d-flex flex-column align-start">
    <v-list-item-content>
      ...
      <div class="d-flex align-center pb-15">
        ...
        <span style="font-size: 0.8em; color: #bfbfbf">{{
          calculateTime(thread.date)
        }}</span>
      </div>
    </v-list-item-content>
    ...
    <v-list-item-content>
      <h4 style="font-weight: bold" class="pt-5">
        ...
        >
          <v-list-item-content>
            <div class="d-flex align-start">
              ...
              <div class="pa-2">
                <v-list-item-title style="font-weight: bold" class="pb-5">{{
                  message.title
                }}</v-list-item-title>
                <v-list-item-text>
                  <div>
                    <span
                      v-if="isTruncated(message.content) && !message.expanded"
                    >
                      {{ truncateText(message.content) }}
                      <a @click="toggleMore(message)" class="more_link"
                        >もっと見る</a
                      >
                    </span>
                    <span v-else>
                      {{ message.content }}
                      <a
                        v-if="isTruncated(message.content)"
                ...
                <div class="d-flex flex-column pb-8 pt-4">
                  ...
                  <div v-else>
                    <a
                      ...
                      download
                      :style="{ color: '#FF5A36', textDecoration: 'underline' }"
                      >{{ message.fileName }}</a
                    >
                  </div>
                  <div class="d-flex align-center">
                    <p class="pr-5 mt-2" style="color: #ff5a36">
                      {{ message.author }}
                    </p>
                    <p
                      v-if="calculateTime(message.date)"
                      style="font-size: 0.8em; color: #bfbfbf"
                      class="mt-2"
                    >
