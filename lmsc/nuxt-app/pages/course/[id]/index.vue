<template>
  <v-row>
    <v-col>
      <LoadingAndError :errors="[courseError, questionsError]" :statuses="[courseStatus, questionsStatus]">
...
const { data: questionsData, error: questionsError, status: questionsStatus } = useGetCurriculumQuestions(curriculumId);

const rawQuestionsData = questionsData.value ? toRaw(questionsData.value.questions) : [];
